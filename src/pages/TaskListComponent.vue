...
<template>
  <div id="lessonTwo">
    ...
    <br><br>
    <input v-model="searchTitle" placeholder="🔎"> <span style="margin:0 10px; color: white">📍Если хотите отредактировать задачу,
    ...
    <ul class="list-group">
      <TodoList v-for="(i, index) in todoList" :key="index" :id="index" @click="i.done = !i.done" :value="i.text"
        ...
        <router-link tag="button" class="btn btn-info" :to="{path:'/tasklist/'+(index+1),query:{title:i.text, statusTask:i.done ? 'Выполнено😍😃' : 'Не выполнено😫'}}">⌽</router-link>
      </TodoList>
